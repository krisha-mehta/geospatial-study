On each trial you will view a map of a different state. Each map is an animation with multiple frames. Each frame represents one plausible estimate of which counties are and are not above the 15% poverty threshold based on a model of survey data. The model is more confident that poverty is acute in counties that are more consistently red.
...
#### Again an example video of the elicitation interface being used for this specific vis condition would be helpful here.
